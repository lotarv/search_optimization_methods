...
def ParticleSwarmAlgorithm(frame,root,ax,canvas):
        ...
        def run_optimization():
            ...
            dimension = 2  # Должно быть 2 для отображения на графике
            ...
            for n in range(iterCount):
                ...
                for particle_instance in swarm:
                    ax.scatter(
                        ...
                        rastrigin(particle_instance.position[0], particle_instance.position[1]),
                        color='red',
                        s=10
                    )

                # Визуализация лучшей частицы
                best = swarm.globalBestPosition
                best_z = rastrigin(best[0], best[1])
